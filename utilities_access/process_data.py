...
def pickle_to_file(batch_num, feedback_data=None):
    ...
    for each_batch in range(curr_batch_num + 1, batch_num + 1):
        for each_sign in range(1, 15):
            ...
            extracted_data_set.append(__emg_feature_extract(raw_data_set)['append_all'])
    ...
    if feedback_data is not None:
        ...
        for sign_id in range(len(feedback_data)):
            ...
            extracted_data_set.append(__emg_feature_extract(sign_raw_data)['append_all'])
...
def main():
    sign_id = 11
    # 从采集文件获取数据
    data_set = Load_ALL_Data(sign_id=sign_id, batch_num=4)
    # 从feedback文件获取数据
